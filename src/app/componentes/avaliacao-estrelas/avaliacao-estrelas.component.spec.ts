...
describe('AvaliacaoEstrelasComponent', () => {
  ...
  // testes sobre effect e sincronização com input signal
  ...
  it('deveria sincronizar signalClassificacao quando classificacao input muda', () => {
    // Cria um novo componente com valor inicial diferente
    ...
    const newFixture = TestBed.createComponent(AvaliacaoEstrelasComponent);
    const newComponente = newFixture.componentInstance;

    // Define o input antes da detecção de mudanças
    newFixture.componentRef.setInput('classificacao', 3);
    newFixture.detectChanges();

    expect(newComponente.valorAtualClassificacao()).toBe(3);
  });

  it('deveria resetar para 1 quando classificacao input recebe valor inválido', () => {
    TestBed.resetTestingModule();
    TestBed.configureTestingModule({
      providers: [
        {
    ...
    newFixture.componentRef.setInput('classificacao', 0);
    newFixture.detectChanges();

    expect(newComponente.valorAtualClassificacao()).toBe(1);
  });

  it('deveria reagir a múltiplas mudanças do input signal classificacao', () => {
    TestBed.resetTestingModule();
    TestBed.configureTestingModule({
      providers: [
        {
    ...
    newFixture.componentRef.setInput('classificacao', 2);
    newFixture.detectChanges();
    expect(newComponente.valorAtualClassificacao()).toBe(2);

    newFixture.componentRef.setInput('classificacao', 5);
    newFixture.detectChanges();
    expect(newComponente.valorAtualClassificacao()).toBe(5);

    newFixture.componentRef.setInput('classificacao', 1);
    newFixture.detectChanges();
    expect(newComponente.valorAtualClassificacao()).toBe(1);
  });
  ...
  // testes sobre input do componente pai
